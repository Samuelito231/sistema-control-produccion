...
@section('content')
    ...
    <div class="w-full min-h-screen flex flex-col">
        <header class="sticky top-0 z-40 bg-black/30 backdrop-blur-md border-b border-white/10 px-8 py-5">
            ...
            <p class="text-gray-400 text-sm mt-1">Mermas y eficiencia en línea de snacks</p>
        </header>
        ...
        <div class="p-8 space-y-8">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-white/5">
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="px-6 py-10 text-center text-gray-400">No hay registros de merma todavía.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <!-- Formulario rápido de registro de merma (visible solo para admin y operario) -->
            @if(in_array(auth()->user()->role, ['admin', 'operario']))
                <div class="bg-black/30 backdrop-blur-md border border-white/10 rounded-2xl p-6">
                    <h3 class="text-lg font-bold text-[#e7c095] mb-4">Registrar nueva merma</h3>
                    <form action="{{ route('produccion.merma.store') }}" method="POST">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <input type="number" step="any" name="cantidad" placeholder="Cantidad (kg/uds)" required
                                   class="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-gray-400">
                            ...
                            <button type="submit" class="bg-gradient-to-r from-[#e7c095] to-[#c29e75] text-black font-bold py-2 rounded-lg shadow-lg hover:shadow-[0_0_15px_rgba(231,192,149,0.5)] transition"
            ...
            @else
                <div class="bg-black/30 backdrop-blur-md border border-white/10 rounded-2xl p-6 text-center text-gray-400">
                    <span class="material-symbols-outlined text-2xl align-middle mr-2">lock</span> No tienes permisos para registrar mermas en producción.
                </div>
            @endif
        </div>
    </div>
