        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/5 transition">
                        ...
                        <td class="px-6 py-4 text-center">
                            @if(in_array(auth()->user()->role, ['admin', 'operario']))
                                <a href="{{ route('produccion.rapida', $producto->id) }}" target="_blank" class="inline-block" title="Escanear QR para merma rápida">
                        ...
                        <td class="px-6 py-4 text-right">
                            @if(auth()->user()->role === 'admin')
                                <a href="{{ route('productos.edit', $producto->id) }}" class="p-1.5 rounded-lg hover:bg-white/10 inline-block" title="Editar">
                            ...
                            @if(auth()->user()->role === 'admin')
                                ...
                                <form action="{{ route('productos.destroy', $producto->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Eliminar? Se conservarán sus mermas.');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="p-1.5 rounded-lg hover:bg-white/10 hover:text-red-400 transition" title="Eliminar">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-10 text-center text-gray-400">No hay productos registrados.<br>Usa el botón "Nuevo producto" para agregar.@{ }
                        <br>Si ya tienes productos, verifica que el filtro no esté ocultándolos.@{
                            </td>
                    </tr>
                    @endforelse
                </tbody>
